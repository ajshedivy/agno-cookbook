    ...
    with Progress(
        ...
        for test_case in tests:
            ...
            try:
                ...
                missing = check_strings(response, test_case.expected_strings, test_case.match_mode)
...
def display_summary(results: list[EvalResult], total_duration: float):
    ...
    if len(agents) > 1:
        ...
        for a in agents:
            ...
            rate_style = "green" if a_rate == 100 else "yellow" if a_rate >= 50 else "red"
            agent_table.add_row(
                a,
                str(a_passed),
                str(a_total),
...
if __name__ == "__main__":
    ...
    passed_count, failed_count, error_count = run_evals(agent=args.agent, category=args.category, verbose=args.verbose)
