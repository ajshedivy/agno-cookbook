...
async def hackernews_research_function(
    ...
    try:
        ...
        response_iterator = hackernews_agent.arun(research_prompt, stream=True, stream_events=True)
...
async def web_search_research_function(
    ...
    try:
        # Stream the agent response
        response_iterator = web_agent.arun(research_prompt, stream=True, stream_events=True)
        async for event in response_iterator:
            yield event
...
async def custom_content_planning_function(
    ...
    try:
        # Stream the agent response
        response_iterator = content_planner.arun(planning_prompt, stream=True, stream_events=True)
        async for event in response_iterator:
            yield event
