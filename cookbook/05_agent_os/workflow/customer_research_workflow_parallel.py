...
def set_session_state_step(step_input: StepInput, run_context: RunContext) -> StepOutput:
    """
    ...
    run_context.session_state["customer_research"]["research_metadata"]["total_research_steps"] += 1
...
async def customer_profile_research_step(
    ...
    try:
        research_result_iterator = customer_profile_agent.arun(research_prompt, stream=True, stream_events=True)
        ...
        yield StepOutput(content=f"Customer profile research failed: {str(e)}", success=False)
...
async def customer_biz_goals_research_step(
    ...
    try:
        research_result_iterator = business_goals_agent.arun(research_prompt, stream=True, stream_events=True)
        ...
        yield StepOutput(content=f"Business goals research failed: {str(e)}", success=False)
...
async def web_intelligence_research_step(
    ...
    try:
        research_result_iterator = web_intelligence_agent.arun(research_prompt, stream=True, stream_events=True)
        ...
        yield StepOutput(content=f"Web intelligence research failed: {str(e)}", success=False)
...
async def customer_report_consolidation_step(
    ...
    try:
        ...
        yield StepOutput(content=f"Research consolidation failed: {str(e)}", success=False)
...
async def task_recommender_step(
    ...
    try:
        ...
        yield StepOutput(content=f"Task recommendation generation failed: {str(e)}", success=False)
