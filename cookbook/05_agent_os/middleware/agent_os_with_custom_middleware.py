...
class RateLimitMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next) -> Response:
        ...
        response.headers["X-RateLimit-Remaining"] = str(self.requests_per_minute - len(history))
...
class RequestLoggingMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next) -> Response:
        ...
        # Basic request info
        client_ip = request.client.host if request.client else "unknown"
        print(f"[REQ] Request #{self.request_count}: {request.method} {request.url.path} from {client_ip}")
        ...
        print(f"{status_label} Response: {response.status_code} in {duration * 1000:.1f}ms")
