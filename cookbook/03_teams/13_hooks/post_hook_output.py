...
def validate_team_response_quality(run_output: TeamRunOutput, team: Team) -> None:
    ...
    team_context += ", ".join(
        [f"{member.name} ({getattr(member, 'description', 'No description')})" for member in team.members]
    )
...
def simple_team_coordination_check(run_output: TeamRunOutput, team: Team) -> None:
    ...
    member_mentions = sum(1 for member in team.members if member.name.lower() in content.lower())
    has_team_language = any(indicator in content.lower() for indicator in team_indicators)
...
def add_collaboration_summary(run_output: TeamRunOutput, team: Team) -> None:
    ...
    if hasattr(run_output, "member_responses") and run_output.member_responses:
        for i, member_response in enumerate(run_output.member_responses):
            member_name = team.members[i].name if i < len(team.members) else f"Member {i + 1}"
...
**Formatted:** {datetime.now().strftime("%Y-%m-%d at %H:%M:%S")}"""
        ...
        print(f"Warning: Advanced team formatting failed ({e}), using collaboration summary")
...
team_simple = Team(
    ...
    members=[
        ...
        Agent(name="Editor", model=model),
    ],
    post_hooks=[simple_team_coordination_check],
    instructions=["Collaborate to create high-quality content with proper writing and editing coordination."],
)
...
metadata_team = Team(
    ...
    instructions=["Provide comprehensive business insights combining market analysis and strategic advice."],
)
...
async def main() -> None:
    ...
    poor_coordination_team = Team(
        ...
        members=[
            Agent(
                name="Agent1",
                model=model,
                instructions=["Give brief, individual responses without considering teammates."],
            ),
            Agent(
                name="Agent2",
                model=model,
    ...
    print("-" * 50)
    metadata_team.print_response(input="What are the key trends in the e-commerce industry for 2024?")
