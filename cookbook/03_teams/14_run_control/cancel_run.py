...
def cancel_after_delay(team: Team, run_id_container: dict, delay_seconds: int = 3) -> None:
    ...
    if run_id:
        ...
        if success:
            ...
            print(f"Failed to cancel team run {run_id} (may not exist or already completed)")
    else:
...
def main() -> None:
    ...
    team_thread = threading.Thread(target=lambda: long_running_task(team, run_id_container), name="TeamRunThread")
