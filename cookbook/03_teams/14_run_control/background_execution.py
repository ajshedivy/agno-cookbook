...
async def example_team_background_run():
    ...
    assert run_output.status == RunStatus.pending, f"Expected PENDING, got {run_output.status}"
...
async def example_cancel_team_background_run():
    ...
    run_output = await team.arun(
        "Write a detailed essay about the history of artificial intelligence. Make it at least 3000 words.",
        background=True,
    )
