...
def analyze_content_strategy(step_input: StepInput) -> StepOutput:
    ...
    if overlap_percentage > 60:
        recommendations.append("HIGH OVERLAP detected - consider a fresh angle or advanced perspective")
    ...
    if len(history_data) > 0:
        recommendations.append(f"Building on {len(history_data)} previous content pieces - ensure progression")
    ...
    strategy_analysis = {
        ...
        "content_gap_analysis": {
            "avoid_repeating": [
                topic for topic in covered_topics if any(word in current_topic.lower() for word in topic.split()[:2])
            ],
