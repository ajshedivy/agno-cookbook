...
class WorkflowWebSocketTester:
    ...
    async def _phase2_reconnect(self) -> None:
        ...
        try:
            async with websockets.connect(self.ws_url) as websocket:
                ...
                print(f"\nSending: reconnect action (run_id={self.run_id}, last_event_index={self.last_event_index})")
                await websocket.send(
                    json.dumps(
                        {
                            "action": "reconnect",
    ...
    def _print_summary(self) -> None:
        ...
        event_indices = [e.get("event_index") for e in self.received_events if "event_index" in e]
        if event_indices:
