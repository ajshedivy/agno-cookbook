...
class WorkflowWebSocketClient:
    ...
    async def connect(self):
        ...
        try:
            ...
            if self.auth_token:
                ...
                self.console.print("[WARN] [yellow]No authentication token provided.[/yellow]")
                self.console.print("[INFO] [blue]Use 'auth' command to authenticate interactively[/blue]")
        ...
        try:
            async for message in self.websocket:
                ...
                try:
                    ...
                    if event_data.get("event") == "connected" or event_data.get("type") == "connection_established":
                        self.connection_id = event_data.get("connection_id")
                    elif event_data.get("event") == "authenticated":
                        self.is_authenticated = True
                        self.console.print("[OK] [green]Authentication successful![/green]")
                    elif event_data.get("event") == "auth_error":
                        self.console.print(f"[ERROR] [red]Authentication failed: {event_data.get('error')}[/red]")
                    ...
                    if event_data:
                        ...
                        if len(message) < 100:
                            self.console.print(f"[ERROR] [red]Could not parse message: {message[:50]}...[/red]")
    ...
    async def start_workflow(self, workflow_message: str, session_id: Optional[str] = None):
        ...
        if not self.is_authenticated and self.auth_token:
            self.console.print("[ERROR] [red]Not authenticated. Please authenticate first.[/red]")
            return
        ...
        self.console.print(f"[START] [blue]Starting workflow with message:[/blue] [bold]{workflow_message}[/bold]")
    ...
    async def run_interactive(self):
        ...
        if not self.is_authenticated:
            if not self.auth_token:
                self.console.print()
                self.console.print("[AUTH] [yellow bold]AUTHENTICATION REQUIRED[/yellow bold]")
                self.console.print("   [yellow]Type 'auth' to authenticate with your token[/yellow]")
            else:
                self.console.print("   [yellow][WARN] Waiting for authentication...[/yellow]")
        self.console.print()
            ...
            while self.is_running:
                try:
                    ...
                    if user_input.lower() in ["quit", "exit", "q"]:
                        ...
                        if not self.is_authenticated:
                            self.console.print("[ERROR] [red]Not authenticated. Use 'auth' command first.[/red]")
                            continue
                        ...
                        if workflow_message:
                            ...
                            self.console.print("[ERROR] [red]Please provide a message for the workflow[/red]")
                    else:
                        self.console.print(
    ...
    async def run_with_message(self, message: str):
        ...
        try:
            self.console.print("[WAIT] [yellow]Listening for workflow events... (Press Ctrl+C to stop)[/yellow]")
            await listen_task
        except KeyboardInterrupt:
