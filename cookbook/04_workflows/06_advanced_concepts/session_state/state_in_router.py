...
friendly_agent = Agent(
    ...
    model=model,
    instructions=("You are a friendly, casual assistant. Use simple language and make the conversation engaging."),
    markdown=True,
)
...
def add_task(run_context: RunContext, task: str, priority: str = "medium") -> str:
    ...
    existing_tasks = [existing_task["name"].lower() for existing_task in run_context.session_state["task_list"]]
...
def list_tasks(run_context: RunContext, status_filter: str = "all") -> str:
    ...
    if "task_list" not in run_context.session_state or not run_context.session_state["task_list"]:
    ...
    tasks_str = "\n".join(
        [f"- [{task['status'].upper()}] {task['name']} (Priority: {task['priority']})" for task in tasks]
    )
...
def clear_completed_tasks(run_context: RunContext) -> str:
    ...
    run_context.session_state["task_list"] = [
        task for task in run_context.session_state["task_list"] if task["status"] != "completed"
    ]
