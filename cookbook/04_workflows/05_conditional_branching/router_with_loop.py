...
def research_quality_check(outputs: List[StepOutput]) -> bool:
    ...
    for output in outputs:
        if output.content and len(output.content) > 300:
            print(f"[PASS] Research quality check passed - found substantial content ({len(output.content)} chars)")
            return True
...
def research_strategy_router(step_input: StepInput) -> List[Step]:
    ...
    if any(keyword in topic for keyword in deep_tech_keywords) or ("tech" in topic and len(topic.split()) > 3):
