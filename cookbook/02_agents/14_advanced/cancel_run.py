...
def cancel_after_delay(agent: Agent, run_id_container: dict, delay_seconds: int = 3):
    ...
    if run_id:
        ...
        if success:
            ...
            print(f"[ERROR] Failed to cancel run {run_id} (may not exist or already completed)")
    else:
...
def main():
    ...
    agent = Agent(
        ...
        model=model,  # Use a model that can generate long responses
    ...
    agent_thread = threading.Thread(target=lambda: long_running_task(agent, run_id_container), name="AgentRunThread")
