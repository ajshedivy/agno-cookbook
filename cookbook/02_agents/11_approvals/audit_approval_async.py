...
async def main():
    ...
    _db = SqliteDb(db_file=DB_FILE, session_table="agent_sessions", approvals_table="approvals")
    _agent = Agent(
        model=model,
        tools=[delete_user_data],
        markdown=True,
    ...
    assert approval_record["status"] == "approved", f"Expected status 'approved', got {approval_record['status']}"
    assert approval_record["approval_type"] == "audit", f"Expected type 'audit', got {approval_record['approval_type']}"
