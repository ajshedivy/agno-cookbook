...
# Create Agent
# ---------------------------------------------------------------------------
db = SqliteDb(db_file=DB_FILE, session_table="agent_sessions", approvals_table="approvals")
agent = Agent(
    model=model,
...
if __name__ == "__main__":
    ...
    assert required_total >= 1, f"Expected at least 1 required approval, got {required_total}"
    ...
    assert required_approval["status"] == "pending", f"Expected status 'pending', got {required_approval['status']}"
    ...
    assert logged_approval["status"] == "approved", f"Expected status 'approved', got {logged_approval['status']}"
    assert logged_approval["approval_type"] == "audit", f"Expected type 'audit', got {logged_approval['approval_type']}"
    ...
    print(f"  Required record: type={required_list[0]['approval_type']}, status={required_list[0]['status']}")
    print(f"  Logged record:   type={logged_list[0]['approval_type']}, status={logged_list[0]['status']}")
