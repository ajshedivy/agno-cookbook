...
def validate_response_quality(run_output: RunOutput) -> None:
    ...
    validation_result = validator_agent.run(input=f"Validate this response: '{run_output.content}'")
...
async def main():
    ...
    agent_simple = Agent(
        ...
        instructions=["You are a helpful assistant. Keep responses focused and appropriate length."],
    )
    ...
    print("-" * 40)
    try:
        await agent_with_validation.aprint_response(input="How do I reset my password on my Microsoft account?")
    ...
    print("-" * 40)
    try:
        await agent_simple.aprint_response(input="Explain what a database is in simple terms.")
