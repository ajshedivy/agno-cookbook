...
agno_embedder = GeminiEmbedder()  # Agno embedder is used to get the embedding for the vector database
chonkie_embedder = GeminiEmbeddings(
    model="gemini-embedding-exp-03-07"
)  # Chonkie embedder is used to get the embedding for the semantic chunking

knowledge = Knowledge(
    vector_db=PgVector(table_name="recipes_semantic_chunking", db_url=db_url, embedder=agno_embedder),
)
knowledge.insert(
