...
def run_sync() -> None:
    ...
    sync_zep_tools = ZepTools(user_id="agno", session_id="agno-session", add_instructions=True)
    ...
    if sync_agent.dependencies:
        # Refresh the context
        sync_agent.dependencies["memory"] = sync_zep_tools.get_zep_memory(memory_type="context")
...
async def run_async() -> None:
    ...
    async_zep_tools = ZepAsyncTools(user_id="agno", session_id="agno-async-session", add_instructions=True)
    ...
    # Refresh the context
    async_agent.dependencies["memory"] = await async_zep_tools.get_zep_memory(memory_type="context")
